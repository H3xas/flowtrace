 ...
 * exactly once, the same walk `cover` then reads. `stale` defaults to the freshness
 * comparison `lib/facts.js` already makes between a fact set's `headSha` and its
 * repository's HEAD, and any entry in it stops the run before a narrowed list exists.
 *
...
export function affected(factSets, options = {}) {
  ...
  // Rung 1 — stale facts. Yesterday's graph selects yesterday's specs, and that failure
  // looks exactly like an answer, so nothing is selected at all.
  if (stale.length > 0) {
    for (const message of stale) widen('stale-index', null, message);
