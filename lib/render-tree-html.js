 ...
 * `pw_test` declaration lines, `pw_title` resolutions by declaration line, and the
 ...
 * collector resolved at that declaration line. Both sides key on `(spec, line)`, so the
 * join evaluates no template. With no `pw_title` fact the expression is kept as written
 * and labelled `raw`, never guessed at.
 */
...
const ANNOTATIONS = [
  ...
  ['mech mech-listed', 'listed', 'title resolved from a pw_title fact, joined on the declaration line'],
...
own source; a <code>pw_title</code> fact carries what Playwright's own collector resolved at that declaration line. On this page
${listed} title${listed === 1 ? '' : 's'} resolved that way and ${raw} kept the expression, marked <em>raw</em>. Nothing here evaluates
a template: both sides key on <code>(spec, line)</code>.
