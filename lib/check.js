 ...
 * rung guards both.
 *
 * Unlike `affected`, `check` refuses on **both** kinds `staleFactsWarnings` reports —
 * `head` and `worktree` alike. A gate compares this run against a committed baseline, and
 * a run that only predates an uncommitted edit still cannot back a regression claim: the
 * baseline was written against a clean tree, and a dirty one is not the same input. Only
 * the message is kept here (`staleFactsWarnings`' own `{message, kind}` shape collapsed
 * to the message alone), because every caller of `result.stale` reads plain strings.
 */
