...
/**
 * The tag a hop carries when an externally supplied fact is stated at its line — set by
 * the caller from the fact sets, never inferred here — so a reader never mistakes such a
 * hop for one the bundled extractor read.
 */
function providerTag(node, paint) {
  return node.provider ? `  ${paint('[provider]', 'cyan')}` : '';
}
...
function renderNodeLine(node, paint) {
    ...
    const aside =
      node.primary === false
        ...
        : node.collapsed
          ...
          : '';
    return `${head}  ${node.text || ''}${outcome}  ${paint(`[branch:${node.branchKind}]`, 'dim')}${providerTag(node, paint)}${aside}`;
  }
  ...
  const summary = node.summary
    ...
    : '';
  return `${crossing}${ref}  ${node.repo}  ${where}  ${via}${providerTag(node, paint)}${suffix}${summary}`;
}
