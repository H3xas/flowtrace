 ...
 * block the snapshot writes. Its `id` is a digest over the producer, the format version and
 * those identities, so a facts change, a re-extraction at another HEAD, a different facts
 * provider or a tool upgrade produces a new id while an unchanged state keeps it.
 *
 * Every fact set the envelope names is accounted for: either some record's end names its
 * repository, or `factSetsWithoutEdges` lists it. A fact set sitting in the envelope as a
 * witness for edges it never produced would otherwise read as evidence behind them.
 *
...
export function exportEdges(flow, factSets, { tool, config } = {}) {
  ...
  const named = new Set(edges.flatMap((record) => [record.from.repo, record.to.repo]));
  const factSetsWithoutEdges = provenance.factSets
    .map((identity) => identity.repo)
    .filter((repo) => !named.has(repo));
  return {
    schemaVersion: EDGES_SCHEMA_VERSION,
    format: EDGES_FORMAT,
    tool: { name: tool?.name ?? 'flowtrace-cli', version: tool?.version ?? '0.0.0' },
    provenance: { ...provenance, factSetsWithoutEdges },
    edges,
  };
}
