...
/**
 * The exact title the runner must report back for a declaration, or `null` when the source
 * text cannot say — the declaration then rests on its ordinal alone.
 *
 * A parsed title arrives already stripped of its quotes, so a plain string and a bare
 * identifier are indistinguishable by content and only their shape separates them. An
 * identifier or member path is a value supplied elsewhere; a template is evaluated; a call is
 * evaluated unless it is a configured case-id wrapper, whose sole string argument survives
 * into the reported title.
 */
const IDENTIFIER_TITLE_RE = /^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*$/;
const STRING_ARG_RE = /^(['"])((?:\\.|(?!\1).)*)\1$/;

export function echoedTitle(expression, caseIdCalls = []) {
  const text = String(expression).trim();
  if (text === '' || text.includes('${')) return null;
  if (IDENTIFIER_TITLE_RE.test(text)) return null;
  const call = /^([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\s*\(([\s\S]*)\)$/.exec(text);
  if (!call) return text;
  if (!caseIdCalls.includes(call[1])) return null;
  const inner = splitTopLevelArgs(call[2]).find((arg) => STRING_ARG_RE.test(arg.trim()));
  if (inner === undefined) return null;
  return STRING_ARG_RE.exec(inner.trim())[2];
}

/** Split a call's argument text on commas that are not inside brackets, quotes or a template. */
 ...
 * title proves the two sequences do not line up.
 *
 * Only a title the runner is obliged to echo back carries that evidence. A template, a
 * table-supplied identifier and an unrecognized call are all evaluated before the runner ever
 * sees them, so the two sides are meant to differ and the ordinal stands alone. A title
 * wrapped in a configured case-id call is the exception: its inner string is still echoed
 * verbatim, so unwrapping recovers the check instead of discarding it.
 *
 * `declarations` are one file's `pw_test` facts in source order, `sites` its ordered
 * declaration sites, `caseIdCalls` the configured case-id callees.
 */
export function refusalReason(declarations, sites, caseIdCalls = []) {
  ...
  for (let ordinal = 0; ordinal < declarations.length; ordinal += 1) {
    ...
    const expected = echoedTitle(expression, caseIdCalls);
    if (expected === null) continue;
    const { titles } = sites[ordinal];
    if (titles.length === 1 && titles[0] === expected) continue;
    return `declaration ${ordinal + 1} reads ${JSON.stringify(expected)}, the listing resolves ${JSON.stringify(titles)} there`;
  }
  return null;
}
 ...
 */
export function titleFacts(facts, sitesBySpec, refusals = [], caseIdCalls = []) {
  ...
  for (const [spec, list] of declarations) {
    ...
    const reason = refusalReason(list, sites, caseIdCalls);
