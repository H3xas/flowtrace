 ...
 * parameter list exactly as they read a method's, and `handler` on the fact says which
  ...
  const emit = (matchIndex, verbWord, template, openParenIndex, templateEndIndex) => {
    ...
    if (handler.action) {
      ...
      methodFrames.push({
        ...
        lambda: { enclosing: enclosingMethod },
      });
    }
    for (const verb of verbs) {
      found.push({
...
/**
 * A lambda handler's lines belong to the lambda. The method that registers it spans the
 * same lines, so the branch and call passes claim them twice; the registering method's
 * copies go, and only the action's own remain.
 */
function dropFactsShadowedByLambdas(relPath, content, methodFrames, facts) {
  const shadows = methodFrames
    .filter((frame) => frame.lambda && frame.lambda.enclosing)
    .map((frame) => ({
      className: frame.lambda.enclosing.classRef.name,
      method: frame.lambda.enclosing.name,
      startLine: lineAt(content, frame.headerIndex),
      endLine: lineAt(content, frame.endIndex),
    }));
  if (shadows.length === 0) return;
  for (let index = facts.length - 1; index >= 0; index -= 1) {
    const entry = facts[index];
    if (entry.file !== relPath || (entry.type !== 'branch_point' && entry.type !== 'method_call')) continue;
    const shadowed = shadows.some(
      (shadow) =>
        entry.class === shadow.className &&
        entry.method === shadow.method &&
        entry.line >= shadow.startLine &&
        entry.line <= shadow.endLine,
    );
    if (shadowed) facts.splice(index, 1);
  }
}
 ...
 * method declared in another one.
 */
...
export async function extract(repoRoot, options = {}) {
  ...
  for (const file of files) {
    ...
    dropFactsShadowedByLambdas(file.relPath, content, methodFrames, facts);
