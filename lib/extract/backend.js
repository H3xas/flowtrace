...
function matchClassHeader(rawHeader) {
  ...
  // The declaration's own modifiers only: the header runs from the previous brace, so an
  // earlier member declaration (`public abstract Task Run();`) is cut off at its semicolon.
  const modifiers = header.slice(0, match.index);
  const ownModifiers = modifiers.slice(modifiers.lastIndexOf(';') + 1);
  return {
    keyword: match[1],
    name: match[2],
    ...
    isAbstract: /\babstract\b/.test(ownModifiers),
  };
}
