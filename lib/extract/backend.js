 ...
 * `IPublishEndpoint.Publish`/`PublishAsync` idiom and its job-service `SubmitJob`. A
 * worker-queue framework with its own publish method names adds them through
 * `workerPatterns.publishCalls`. */
const DEFAULT_PUBLISH_CALLS = Object.freeze(['Publish', 'SubmitJob']);
...
function compileWorkerPatterns(overrides = {}) {
  ...
  return {
    ...
    /** Four spellings of a publish call, the message in one capture group each: the generic
     * argument `Publish<T>(…)`, the inline `Publish(new T …)`, the saga initialiser
     * `PublishAsync(ctx => ctx.Init<T>(…))`, and a bare identifier as the first argument
     * (`Publish(job, ct)`), which `extractPublish` resolves to its declared type. */
    publishCall: new RegExp(
      `\\b(?:${publish})(?:Async)?\\s*<\\s*(\\w+)\\s*>` +
        `|\\b(?:${publish})(?:Async)?\\s*\\(\\s*(?:await\\s+)?new\\s+(\\w+)\\b` +
        `|\\b(?:${publish})(?:Async)?\\s*\\(\\s*\\w+\\s*=>\\s*\\w+\\s*\\.\\s*Init\\s*<\\s*(\\w+)\\s*>` +
        `|\\b(?:${publish})(?:Async)?\\s*\\(\\s*(?:await\\s+)?([A-Za-z_]\\w*)\\s*[,)]`,
      'g',
    ),
...
/**
 * One `publish` per publish-verb call: the message is the generic argument (`Publish<T>(…)`),
 * the inline `new T`, the saga initialiser (`PublishAsync(ctx => ctx.Init<T>(…))`), or the type
 * of a bare identifier argument resolved by `resolveArgumentType`. A site whose identifier's
 * type cannot be read still emits, with `message: null` and `unresolved` naming why. A generic
 * type parameter (`TMessage`) is never a message, however the call spells it.
 */
function extractPublish(relPath, content, masked, facts, messageIndex, classFrames, methodFrames, expressionBodiedMethods, patterns) {
  ...
  while ((match = patterns.publishCall.exec(masked))) {
    let message = match[1] || match[2] || match[3] || null;
    let unresolved = null;
    if (!message) {
