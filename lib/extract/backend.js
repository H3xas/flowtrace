 ...
 * message it consumes. Omitted for a parameterless method.
 */
function paramTypesField(paramsText) {
  const types = parseParams(paramsText).map((param) => param.type);
  ...
  for (const method of methodFrames) {
    ...
    facts.push(
      fact('method_span', {
        ...
        class: method.classRef.name,
        method: method.name,
        ...paramTypesField(method.paramsText),
      }),
    );
  }
}
  ...
  for (const entry of expressionBodiedMethods) {
    facts.push(
      fact('method_span', {
        ...
        method: entry.name,
        ...paramTypesField(entry.paramsText),
      }),
    );
  }
}
