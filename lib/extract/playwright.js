...
/**
 * A declaration always ends in the function that holds the test body; the two-argument
 * `test.skip(condition, reason)` form is a runtime guard written inside a body and ends in a
 * string. Shape alone separates them, so nothing here evaluates an expression.
 */
function endsInFunction(text) {
  const trimmed = String(text).trim();
  const body = /^async\b/.test(trimmed) ? trimmed.slice(5).trim() : trimmed;
  if (/^function\b/.test(body)) return true;
  if (/^[A-Za-z_$][\w$]*\s*=>/.test(body)) return true;
  if (body[0] === '(') {
    const close = matchBalanced(body, 0);
...
function findTestBlocks(code) {
  ...
  while ((m = TEST_RE.exec(code))) {
    ...
    if (parts.length < 2 || !endsInFunction(parts[parts.length - 1].text)) continue;
