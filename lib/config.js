 ...
 * framework's own registration, publish, consumer-base, saga-interface, topology-binding,
 * queue-prefix and config-read patterns, since only public idioms are built in, and the
 * trace step with the framework's own consumer entry-method names. Optional per-repository
 * `factsProvider` names an external fact document — a file, or a command that prints
 * one — and the mode its facts merge with the extraction under. Optional
 * `scope.gatePatterns` names the node-ref patterns `scope` reads as an authorization
 * point, defaulting to a built-in naming heuristic.
 *
...
import { PROVIDER_MERGE_MODES } from './facts.js';

export const CONFIG_FILENAME = 'flowtrace.config.json';
/** The `factsProvider` fields: exactly one of `file` or `command`, plus a required `merge`. */
export const FACTS_PROVIDER_FIELDS = Object.freeze(['file', 'command', 'merge']);
...
/**
 * `scope`'s built-in `gatePatterns`: bare name fragments matched against a walked node's
 * `ref`. A naming heuristic, not a guarantee — it both misses an authorization point named
 * something else and over-reports a method whose name happens to contain one of these
 * words without enforcing anything. `docs/configuration.md` states the same caution.
 */
export const DEFAULT_GATE_PATTERNS = Object.freeze(['Authorize', 'Permission', 'Policy', 'Entitlement', 'Claims']);
/** The `scope` configuration object's only recognised field. */
export const SCOPE_FIELDS = Object.freeze(['gatePatterns']);
...
/**
 * The `workerPatterns` fields the backend extractor understands, plus
 * `consumerEntryMethods`, which the trace step reads. Every field is an array of
 * strings validated as regex sources, except `consumerEntryMethods`, which holds plain
 * method names and is validated as such, and `publishCalls`, whose entries are a verb's
 * regex source with an optional `/N` suffix (see below); `docs/configuration.md` describes
 * what each one matches and which capture groups the whole-regex fields must expose.
 */
export const WORKER_PATTERN_FIELDS = Object.freeze([
  'registrations', 'topologyBindings', 'queuePrefixConstants',
  'publishCalls', 'consumerBases', 'broadcastCalls', 'configReads', 'consumerEntryMethods',
...
const SCAFFOLD_FIELDS = Object.freeze([
  ...
  'caseIdPlaceholder', 'poll', 'worker', 'clientClassTemplate', 'unresolvedRejectStatus', 'okStatus',
]);
const SCAFFOLD_STRING_FIELDS = Object.freeze([
  'specRoot', 'specSuffix', 'workerRoot', 'caseIdPlaceholder', 'clientClassTemplate',
]);
const SCAFFOLD_IMPORT_ALIAS_FIELDS = Object.freeze(['caseId', 'fixtures', 'utils', 'features', 'generated']);
...
const SCAFFOLD_POLL_FIELDS = Object.freeze(['timeoutMs', 'intervalsMs']);
...
function validateScaffold(value, file) {
  ...
  if (raw.poll !== undefined) {
    const poll = requireObject(raw.poll, 'scaffold.poll', file);
    rejectUnknownFields(poll, 'scaffold.poll', SCAFFOLD_POLL_FIELDS, file);
    const validated = {};
    if (poll.timeoutMs !== undefined) {
      if (!isPositiveInteger(poll.timeoutMs)) {
        fail(file, '"scaffold.poll.timeoutMs" must be a positive integer');
      }
      validated.timeoutMs = poll.timeoutMs;
    }
    if (poll.intervalsMs !== undefined) {
      if (
        !Array.isArray(poll.intervalsMs) ||
        poll.intervalsMs.length === 0 ||
        poll.intervalsMs.some((interval) => !isPositiveInteger(interval))
      ) {
        fail(file, '"scaffold.poll.intervalsMs" must be a non-empty array of positive integers');
      }
      validated.intervalsMs = poll.intervalsMs.slice();
    }
    scaffold.poll = validated;
  }
...
/**
 * `scope.gatePatterns`: whole regexes matched against a walked node's `ref`, validated
 * the same way `workerPatterns`' regex-source fields are — a non-empty string that
 * compiles — and defaulting to `DEFAULT_GATE_PATTERNS` when the key is absent, so a run
 * that never configures one still reports something rather than `none` everywhere.
 */
function validateScope(value, file) {
  if (value === undefined) return { gatePatterns: [...DEFAULT_GATE_PATTERNS] };
  const raw = requireObject(value, 'scope', file);
  rejectUnknownFields(raw, 'scope', SCOPE_FIELDS, file);
  if (raw.gatePatterns === undefined) return { gatePatterns: [...DEFAULT_GATE_PATTERNS] };
  if (
    !Array.isArray(raw.gatePatterns) ||
    raw.gatePatterns.some((entry) => typeof entry !== 'string' || entry.trim() === '')
  ) {
    fail(file, '"scope.gatePatterns" must be an array of non-empty strings');
  }
  const gatePatterns = raw.gatePatterns.map((entry) => {
    const source = entry.trim();
    try {
      new RegExp(source);
    } catch (error) {
      fail(file, `"scope.gatePatterns" entry ${JSON.stringify(source)} is not a valid regular expression (${error.message})`);
    }
    return source;
  });
  return { gatePatterns };
...
export function validateConfig(raw, { baseDir = process.cwd(), file = CONFIG_FILENAME, env = process.env } = {}) {
  ...
  // `split` drafts commit subjects; a configured prefix goes in front of each one.
  let split = null;
  if (raw.split !== undefined) {
    const splitRaw = requireObject(raw.split, 'split', file);
    rejectUnknownFields(splitRaw, 'split', ['ticketPrefix'], file);
    split = {};
    if (splitRaw.ticketPrefix !== undefined) {
      const prefix = requireString(splitRaw.ticketPrefix, 'split.ticketPrefix', file);
      if (prefix.trim() === '' || /\s/.test(prefix)) {
        fail(file, '"split.ticketPrefix" must be a non-empty string without whitespace');
      }
      split.ticketPrefix = prefix;
    }
  }
  let scout = null;
  ...
  const scope = validateScope(raw.scope, file);
  ...
  if (raw.workerPatterns !== undefined) {
    ...
    for (const field of Object.keys(raw.workerPatterns)) {
      ...
      workerPatterns[field] = values.map((value) => {
        ...
        if (field === 'consumerEntryMethods' && !/^\w+$/.test(source)) {
          fail(file, `"workerPatterns.consumerEntryMethods" entry ${JSON.stringify(source)} is not a method name (letters, digits and underscores only)`);
        }
        // A `publishCalls` entry may name the 1-based index of its message argument after a
        // slash (`Defer/2`); a bare verb keeps meaning index 1. Only the verb half is a regex
        // source -- the index half is checked here and never reaches `new RegExp` below.
        let regexSource = source;
        if (field === 'publishCalls' && source.includes('/')) {
          const slash = source.indexOf('/');
          const verb = source.slice(0, slash);
          const index = source.slice(slash + 1);
          if (verb === '' || index.includes('/') || !/^[1-9]\d*$/.test(index)) {
            fail(file, `"workerPatterns.publishCalls" entry ${JSON.stringify(source)} is not a valid "Verb" or "Verb/N" form (N must be a positive integer)`);
          }
          regexSource = verb;
        }
        try {
  ...
  const repos = raw.repos.map((entry, index) => {
    ...
    if (entry.titles !== undefined) {
      if (typeof entry.titles !== 'boolean') fail(file, `"${at}.titles" must be true or false`);
      if (kind !== 'playwright') fail(file, `"${at}.titles" applies to a playwright repository only (kind "${kind}")`);
    }
    let role = [];
    ...
    if (entry.factsProvider !== undefined) {
      const raw = requireObject(entry.factsProvider, `${at}.factsProvider`, file);
      rejectUnknownFields(raw, `${at}.factsProvider`, FACTS_PROVIDER_FIELDS, file);
      if ((raw.file === undefined) === (raw.command === undefined)) {
        fail(file, `"${at}.factsProvider" must name exactly one of "file" or "command"`);
      }
      // The mode is required on purpose: it changes every number downstream, and a
      // silent default would make two configurations with different intent look the same.
      if (raw.merge === undefined) {
        fail(file, `"${at}.factsProvider.merge" is required (one of ${PROVIDER_MERGE_MODES.join(', ')})`);
      }
      if (!PROVIDER_MERGE_MODES.includes(raw.merge)) {
        fail(file, `"${at}.factsProvider.merge" must be one of ${PROVIDER_MERGE_MODES.join(', ')} (got "${String(raw.merge)}")`);
      }
      if (raw.file !== undefined) {
        factsProvider = {
          source: 'file',
          file: resolve(baseDir, requireString(raw.file, `${at}.factsProvider.file`, file)),
          merge: raw.merge,
        };
      } else {
        if (
          ...
          raw.command.some((value) => typeof value !== 'string' || value.trim() === '')
        ) {
          fail(file, `"${at}.factsProvider.command" must be a non-empty array of non-empty strings`);
        }
        const [bin, ...args] = raw.command.map((value) => value.trim());
        factsProvider = { source: 'command', command: [resolveBinValue(bin, baseDir), ...args], merge: raw.merge };
      }
    }
    return {
      id, kind, root: resolve(baseDir, root), exclude, scout: entry.scout === true, titles: entry.titles === true, role,
      featureRoots, srcSubpath, cypressSubpath, cypress, factsProvider,
    };
  });
  return { file, out: resolve(baseDir, out), repos, aliases, scout, sinks, caseId, workerPatterns, scaffold, split, scope };
}
