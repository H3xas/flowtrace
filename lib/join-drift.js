...
import { factsDigest, validateFacts } from './facts.js';
import { join, normalizeAliases } from './join.js';
import { routeKey } from './normalize.js';
import { MAX_SEEDS, NOMINAL_DISPOSITIONS, trace } from './trace.js';
...
export const FINDING_FAMILIES = Object.freeze({
  path: Object.freeze(['route-added', 'route-removed', 'path-added', 'path-removed', 'route-reshaped', 'call-unserved']),
...
/**
 * The fields of a header's `provider` block that identify what a second producer
 * contributed. `source` is left out: it holds the provider file or command as configuration
 * resolved it, an absolute path, so two checkouts of one revision at different locations
 * would disagree on an identity their facts agree on, and the identity is published in
 * snapshots and edge exports. `digest` covers the provider's own facts instead, which is
 * what tells two documents apart under `regex-only-with-diff`, where none of them reaches
 * the fact array the set's own digest is taken over.
 */
...
function providerIdentity(provider) {
  ...
  for (const field of PROVIDER_IDENTITY_FIELDS) {
    if (provider[field] !== undefined) identity[field] = provider[field];
  }
  return identity;
}

/**
 * The identity block of one fact set, plus the digest of its facts. `headSha` is always
 * stated: a set whose header carries no revision witness, from a root that is not a git
 * checkout, says `null` rather than omitting the field, so an absent witness can never be
 * read as a field this identity did not think to carry.
 */
export function factSetIdentity(set) {
  const identity = {};
  for (const field of IDENTITY_FIELDS) {
    if (set[field] !== undefined && set[field] !== null) identity[field] = set[field];
    else if (field === 'headSha') identity.headSha = null;
  }
  const provider = providerIdentity(set.provider);
...
export function readSnapshot(parsed, name = 'snapshot') {
  ...
  if (parsed.schemaVersion === 1) {
    throw new SnapshotError(
      `${name}: schemaVersion 1 is not supported (this version reads ${SNAPSHOT_SCHEMA_VERSION}): a fact set's ` +
        'identity now carries its facts provider and states an absent revision witness, which a ' +
        'schemaVersion 1 snapshot cannot; retake the snapshot with join --snapshot',
    );
  }
...
function compareBehaviour(before, after, pairs, findings, skipped) {
  for (const pair of pairs) {
    ...
    if (left.truncated || right.truncated) {
      skipped.push({ ...base, reason: truncatedReason(left, right) });
      continue;
    }
...
/**
 * Why a route's seed, effect and evidence comparison was skipped rather than run: which
 * side's walk hit the seed cap, named so a reader never has to re-run the walk to find out.
 */
function truncatedReason(left, right) {
  const sides = [];
  if (left.truncated) sides.push('the snapshot side');
  if (right.truncated) sides.push('the current side');
  const subject = sides.length > 1 ? `${sides.join(' and ')}'s seed walks were` : `${sides[0]}'s seed walk was`;
  return `${subject} truncated at the ${MAX_SEEDS}-seed cap; seeds, effects and evidence were not compared`;
}
