...
const CONSUMER_METHODS = ['Consume', 'Process', 'ProcessAsync'];
...
export function resolveStart(factSets, start, options = {}) {
  const index = options.index || buildIndex(factSets);
...
export function trace(factSets, start, options = {}) {
  ...
  function attach(parent, node, via) {
    node.via = via;
  ...
  function child(parent, fields, via) {
  ...
  function expandMessage(node, hops) {
    ...
    for (const { entry, level } of consumers) {
      ...
      consumer.hops = 0;
      asNewEntryPoint(() => expandClass(consumer, 0, CONSUMER_METHODS));
    }
    ...
    for (const { entry, level } of processors) {
      ...
      processor.hops = 0;
      asNewEntryPoint(() => expandClass(processor, 0, CONSUMER_METHODS));
    }
    release(node);
  }
  ...
  function expandCalls(node, className, methodName, hops) {
    ...
    for (const entry of calls) {
      ...
      const self = selfCalls.get(called) || { name: called, line: entry.fact.line, lines: [] };
    ...
    for (const entry of receiverCalls) {
      ...
      if (!group.methods.some((called) => called.name === entry.fact.calledMethod)) {
        group.methods.push({ name: entry.fact.calledMethod, line: entry.fact.line, lines: [] });
      }
    ...
    for (const item of ordered) {
      ...
      if (elementType && isInterfaceName(elementType)) {
        ...
        const kept = impls.slice(0, INTERFACE_FANOUT_CAP);
        if (kept.length === 0) {
  ...
  function expandMobileMethod(node, hops) {
    ...
    for (const { fact } of calls) {
      ...
      if (!group.methods.some((called) => called.name === fact.calledMethod)) {
        group.methods.push({ name: fact.calledMethod, line: fact.line });
      }
   ...
   */
  function addPropHop(parent, componentName, propName, fact, event) {
  ...
  function expandWebMethod(node, hops) {
    ...
    for (const { fact } of calls) {
      ...
      seen.add(key);
      if (fact.field === 'props') {
        addPropHop(node, node.class, fact.calledMethod, fact, null);
        continue;
      }
      if (fact.field === 'this') {
  ...
  function expand(node, hops) {
    switch (node.kind) {
      ...
      case 'consumer':
      case 'processor':
        expandClass(node, hops, CONSUMER_METHODS);
        break;
      case 'class':
