...
/**
 * Whether a parameter type, as written, names a type: `ConsumeContext<T>`, `JobContext<T>`,
 * `Batch<T>`, `ConsumeContext<Batch<T>>`, a namespace-qualified `Messages.T` or a bare `T`
 * all do, because the name is a whole identifier somewhere in the type text.
 */
function namesType(paramType, name) {
  return String(paramType || '').split(/[^\w]+/).includes(name);
}
...
export function trace(factSets, start, options = {}) {
   ...
   * every declared method whose parameter types name that message. Only when no
